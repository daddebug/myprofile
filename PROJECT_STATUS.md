...
- **Template-instance image publishing**: images referenced by `imageId` (image-row, direction-compare, etc.) are staged locally by the dev server under `public/portfolio-assets/` and only ever have a real, working URL on the editing machine. `productionBundleExport.ts`/`import-production-bundle.mjs` now fetch these from that local URL during export and copy them into `public/images/published/template-images-<projectId>/`, rewriting `publicPath` the same way `localImageId` references already were. Verified live: every image on `project-1ua2677`, `game-jam-8lzejf`, and `ui-16ienxj` returns real image bytes.
