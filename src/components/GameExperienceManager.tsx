...
import { ArrowDown, ArrowUp, ImagePlus, Pencil, Plus, X } from "lucide-react";
...
export function GameExperienceManager({ locale, store, initialEditingId = null, startWithNewGame = false, onClose }: Props) {
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      saveGameExperienceStore({ ...next, records: normalizedRecords.map((record) => ({ ...record, publication: { ...record.publication, homepageOrder: homepageOrderById.get(record.id) ?? record.publication.homepageOrder } })) });
      onClose();
    } catch (error) {
      ...
      <div className="mx-auto max-w-7xl rounded-lg bg-[#111846] p-5 shadow-2xl md:p-8">
        ...
        <div className="mt-6 grid gap-3">
          {displayedRecords.map((record) => {
            ...
            return <ManagerRow key={record.id} record={record} locale={locale} selectedCount={selectedCount} limit={draft.homepageLimit} onEdit={() => { setEditingId(record.id); setEditorOriginal(structuredClone(record)); setPendingCover(null); setCoverError(""); }} onMove={(direction) => reorder(record.id, direction)} onPatch={(patch) => patchRecord(record.id, patch)} disableUp={index === 0} disableDown={index === records.length - 1} />;
          })}
        </div>
      </div>
...
function ManagerRow({ record, locale, selectedCount, limit, onEdit, onMove, onPatch, disableUp, disableDown }: { record: GameExperienceRecord; locale: Locale; selectedCount: number; limit: number; onEdit: () => void; onMove: (direction: -1 | 1) => void; onPatch: (patch: (record: GameExperienceRecord) => GameExperienceRecord) => void; disableUp: boolean; disableDown: boolean }) {
...
function GameEditorForm({ record, locale, pendingCover, pendingCoverPreview, coverError, onCover, onPatch }: { record: GameExperienceRecord; locale: Locale; pendingCover: File | null; pendingCoverPreview: string | null; coverError: string; onCover: (file: File | null) => void; onPatch: (patch: (record: GameExperienceRecord) => GameExperienceRecord) => void }) {
  ...
  const area = (label: string, value: string, update: (value: string) => void) => <label className="block text-xs text-softWhite/52"><span className="mb-1.5 block">{label}</span><textarea rows={3} value={value} onChange={(event) => update(event.target.value)} className="w-full rounded bg-[#0d143f] px-3 py-2.5 text-sm leading-6 text-softWhite" /></label>;
  ...
  const patchDetail = (key: keyof GameExperienceRecord["detail"], value: string) => onPatch((item) => ({ ...item, detail: { ...item.detail, [key]: value } }));
  return <div className="mt-6 space-y-7">
    ...
    <section className="grid gap-4 sm:grid-cols-2">{field(locale === "zh" ? "游玩小时" : "Playtime hours", record.stats.playtimeHours, (value) => onPatch((item) => ({ ...item, stats: { ...item.stats, playtimeHours: value ? Math.max(0, Number(value)) : undefined } })), "number")}{field(locale === "zh" ? "游玩时长显示（中文）" : "Playtime label (Chinese)", record.stats.playtimeLabelZh, (value) => onPatch((item) => ({ ...item, stats: { ...item.stats, playtimeLabelZh: value } })))}<label className="block text-xs text-softWhite/52"><span className="mb-1.5 block">{locale === "zh" ? "成就状态" : "Achievement state"}</span><select value={record.stats.achievementMode} onChange={(event) => onPatch((item) => ({ ...item, stats: { ...item.stats, achievementMode: event.target.value as GameExperienceRecord["stats"]["achievementMode"] } }))} className="w-full rounded bg-[#0d143f] px-3 py-2.5 text-sm text-softWhite"><option value="exact">Exact unlocked / total</option><option value="percentage">Percentage only</option><option value="unavailable">Not recorded</option><option value="none">No achievements</option></select></label>{record.stats.achievementMode === "exact" ? <div className="grid grid-cols-2 gap-2">{field(locale === "zh" ? "已解锁" : "Unlocked", record.stats.achievementUnlocked, (value) => onPatch((item) => ({ ...item, stats: { ...item.stats, achievementUnlocked: value ? Number(value) : undefined } })), "number")}{field(locale === "zh" ? "总数" : "Total", record.stats.achievementTotal, (value) => onPatch((item) => ({ ...item, stats: { ...item.stats, achievementTotal: value ? Number(value) : undefined } })), "number")}</div> : record.stats.achievementMode === "percentage" ? field(locale === "zh" ? "完成百分比" : "Percentage", record.stats.achievementPercent, (value) => onPatch((item) => ({ ...item, stats: { ...item.stats, achievementPercent: value ? Number(value) : undefined } })), "number") : null}</section>
    <section>
    ...
    <section className="grid gap-5 md:grid-cols-2"><div className="space-y-4"><h4 className="font-display text-xl text-softWhite">中文反思</h4>{area("为什么游玩", record.reflection.whyPlayedZh, (value) => patchReflection("whyPlayedZh", value))}{area("优点", record.reflection.strengthsZh, (value) => patchReflection("strengthsZh", value))}{area("缺点", record.reflection.weaknessesZh, (value) => patchReflection("weaknessesZh", value))}{area("对我的帮助", record.reflection.contributionZh, (value) => patchReflection("contributionZh", value))}</div><div className="space-y-4"><h4 className="font-display text-xl text-softWhite">English reflection</h4>{area("Why I played", record.reflection.whyPlayedEn, (value) => patchReflection("whyPlayedEn", value))}{area("Strengths", record.reflection.strengthsEn, (value) => patchReflection("strengthsEn", value))}{area("Weaknesses", record.reflection.weaknessesEn, (value) => patchReflection("weaknessesEn", value))}{area("What I learned", record.reflection.contributionEn, (value) => patchReflection("contributionEn", value))}</div></section>
    <section className="border-t border-softWhite/10 pt-6">
      <h4 className="font-display text-xl font-semibold text-softWhite">{locale === "zh" ? "游戏评价详情" : "Full game reflection"}</h4>
      <p className="mt-1.5 text-xs leading-5 text-softWhite/45">{locale === "zh" ? "独立长评正文；留空时当前语言不会显示“查看详情”。" : "Independent long-form copy. Leave a locale empty to hide its View details control."}</p>
      <div className="mt-4 grid gap-5 md:grid-cols-2">
        <label className="block text-xs text-softWhite/52"><span className="mb-1.5 block">游戏评价详情</span><textarea rows={8} value={record.detail.zh} onChange={(event) => patchDetail("zh", event.target.value)} className="w-full rounded bg-[#0d143f] px-3 py-2.5 text-sm leading-6 text-softWhite" /></label>
        <label className="block text-xs text-softWhite/52"><span className="mb-1.5 block">Full game reflection</span><textarea rows={8} value={record.detail.en} onChange={(event) => patchDetail("en", event.target.value)} className="w-full rounded bg-[#0d143f] px-3 py-2.5 text-sm leading-6 text-softWhite" /></label>
      </div>
    </section>
  </div>;
}
