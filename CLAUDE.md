...
- **PORTFOLIO TYPOGRAPHY RULE.** Public portfolio UI must use the existing project sans-serif typography system. Do not introduce new serif/display font families for individual components. New navigation, tabs, buttons, labels, metadata, and interface text must inherit or reuse the existing global font stack/utilities. Chinese UI text must not fall back to Song/serif-style fonts when the surrounding interface uses sans-serif — this happens when a component uses `font-mono` (`tailwind.config.ts`'s mono stack has no CJK-covering font and ends in the bare `monospace` generic, which many systems substitute with a serif CJK font for unmatched glyphs) instead of the body's default sans stack (`font-sans` / `Inter, Avenir Next, Segoe UI, Arial, sans-serif`, declared on `body` in `src/styles.css`), which does correctly resolve CJK to a sans font. Before adding a new font-family declaration, first verify whether an existing typography token/class already covers the use case.
